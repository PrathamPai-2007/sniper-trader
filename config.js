...
// Central configuration loader and validator: reads env/.env, applies strategy presets,
// constructs runtime session paths, and exposes constants used across the project.
...
const MAX_TRACKED_MINTS = 5_000;

const TAKE_PROFIT_MULTIPLES = [1.5];
const TAKE_PROFIT_FRACTION = 0.6;
...
const PUMP_FUN_PROGRAM_ID = address('6EF8rrecth7QZ77z27Y9RQmP22JdK89pX6X1N1B8bN2');
const RAYDIUM_AMM_V4_PROGRAM_ID = address('675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8');
const METEORA_DLMM_PROGRAM_ID = address('LBUZPB6S7GrLs9haYv94LMc6Zz21U9F7r5Q7oJCcG6T');
 ...
 * @param {string} name - The name of the environment variable.
 * @param {number} fallback - The fallback value if the environment variable is not set.
 * @returns {number} The parsed number or the fallback.
 * @throws {Error} If the environment variable exists but is not a valid number.
 */
 ...
 * @param {string} name - The name of the environment variable.
 * @param {boolean} fallback - The fallback value if the environment variable is not set.
...
function loadConfig() {
  ...
  const preset = STRATEGY_PRESETS[strategyName] || STRATEGY_PRESETS.standard;
  ...
  return {
    ...
    buyAmountLamports: decimalToAtomic(buyAmountText, 9),
 ...
 * @param {Object} [config=null] - The configuration object to validate.
 * @returns {boolean} True if the configuration is valid.
 * @throws {Error} If validation errors are found.
 */
function validateStartupConfig(config = null) {

...
// Paper trade replay analyzer: reads session logs, reconstructs trades, and replays
// them through a 7-parameter grid (9,216 combos) to find optimal exit settings.
...
function findSessionDirs(baseDir) {
...
function ingestSessions(baseDir) {
  ...
  for (const sessionDir of sessions) {
    ...
    // Also check old .json extension for backward compatibility
    ...
    // Build trade history lookup by mint
    const tradeByMint = new Map();
    ...
    for (const [mint, events] of eventsByMint) {
      ...
      // Ensure highestPriceUsd is at least the max of all event prices
...
function inferPricePath(trade, numPoints = 20) {
  // Build a synthetic price path: entry → highest → exit
  // This is approximate but sufficient for directional comparison
  ...
  // Determine when the peak occurred (assume midpoint if unknown)
  const peakRatio = 0.4; // peak at 40% of hold time
...
function replayTrade(trade, params) {
  ...
  let remainingTokens = 1.0; // fraction of initial tokens
  let totalProceeds = 0;
  ...
  const earlyGuardSec = 15; // matches default earlyPerformanceGuardSeconds
  ...
  const activationMultiple = 1 + 0.5 * (tpMultiples[0] - 1); // midpoint of first target
  const activationPrice = entryP * Math.min(activationMultiple, 1.12);

  for (let i = 0; i < pricePath.length; i++) {
    const p = pricePath[i].price;
    const t = pricePath[i].time;
    ...
    while (targetsHit < tpMultiples.length) {
      const targetP = entryP * tpMultiples[targetsHit];
  ...
  // Remaining tokens valued at exit price
  if (remainingTokens > 0.001) {
    const exitP = pricePath[pricePath.length - 1].price;
    totalProceeds += originalCost * remainingTokens * (exitP / entryP);
    remainingTokens = 0;
...
function generateGrid() {
  ...
  function* cartesian(arrs, idx = 0, current = []) {
    if (idx === arrs.length) {
      const params = {};
      for (let i = 0; i < keys.length; i++) {
        params[keys[i]] = current[i];
      }
      yield params;
      return;
    }
    for (const v of arrs[idx]) {
...
function runAnalysis(trades, metrics) {
...
function printTradeSummary(trades) {
...
function printTopCombos(results, topN = 15) {
...
function printSensitivity(results, _trades) {
...
function printFunnel(metrics) {
  ...
  // Show funnel pass-through rates
...
function main() {
  ...
  printSensitivity(results, trades);
  printFunnel(metrics);

  // Write full results to file for further analysis
...
function computeSensitivitySummary(results) {
  const summary = {};
